...
mod court_service {
    ...
    fn a_killed_court_resumes_and_still_refuses_replay() {
        ...
        drop(book);
        let restored = Arc::new(RwLock::new(RewardBook::new()));
        ...
        {
            let mut guard = restored.write().expect("book");
    ...
    fn federation_carries_work_once_across_real_sockets() {
        ...
        let book_b = Arc::new(RwLock::new(RewardBook::new()));
        ...
        let book_a = Arc::new(RwLock::new(RewardBook::new()));
        credit_triangle(&mut book_a.write().expect("book"), 2);
        ...
        loop {
            if book_b.read().expect("book").act_len() == 1 {
                break;
            }
        ...
        assert_eq!(
            book_b.read().expect("book").act_len(),
            1,
