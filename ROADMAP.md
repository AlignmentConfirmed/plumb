...
Current state at v0.1.0: the wire, the physics, the court, the SDK, the
signature layer, the `plumbd` node daemon, durable court federation
(`court_store` snapshots, `court_live` replay-safe merge), the IS-4
witness, and the declared-domain universal checker are built and tested.
A local proofnet stands the full economy up on one machine. The
remaining work is the public beta: a testnet genesis, an onboarding kit,
and consumer-facing CI.

Each item completes with a test or a shipped artifact. IDs are defined
in `IMPLEMENTATION.md` (S = signatures, UC = universal checker,
X = x402).

## Phase A — trust (complete)
...
Crypto dependencies enter here as a separate leaf crate; `isthmus`
remains dependency-free.

## Phase B — the node (complete)
...
| **N2** | IS-2 §6 session freshness as IS-2/2 | vectors; replayed session refused |
...
## Phase C — protocol completion (complete)
...
## Phase C+ — the universal checker (complete)

The engine treats domains as data: they circulate as data, and the
binary does not recompile to acquire a discipline. Scheduled ahead of
the beta so testers onboard onto the declared-domain engine rather
than the legacy compiled path. Full ladder: `IMPLEMENTATION.md`.
...
B0 (local proofnet) is complete; B1–B3 remain.

| ID | task | done when |
|---|---|---|
| **B0** | Local proofnet: the full economy on one machine — signed, declared-domain, federated, kill/resume | one script stands it up; the full loop settles locally |
...
Testnet resets are permitted and will be announced.
...
- **O1–O4** — the optimization market (`IMPLEMENTATION.md` §6b):
  ...
  and the homology certificate as the quality tier. T2 unchanged:
  a tighter chain is new work by content address.

- **X1–X5** — x402 payment rails (`IMPLEMENTATION.md`),
  gated behind the settlement receipts S1–S7 enable.
...
A contribution that needs no Rust: implement IS-1 from `protocols/`
and `conformance/` alone and file every ambiguity encountered. The
first independent reading closes Known Gap #5.
