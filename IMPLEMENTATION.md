...
| **K3** | The kernel daemon (`crates/kernel`, `plumb-kernel`): attach via sdk, hear query announcements (tag 85), decode the conjecture, derive, submit the attested proof, take the receipt (tag 81) — looping, native Plumbline, no HTTP | **DONE** — a standalone crate, zero `datum` in its manifest (enforced by `tests/no_court_dependency.rs`), speaking plain Plumbline directly against isthmus's session/frame/hello primitives; live-validated against a real plumb-datum court (not a mock): an unregistered `kernel-1` holder derived `bab = aa` from the announced conjecture alone and was credited real axes on the first round, and correctly refused-as-replay on a resubmission |
| **K4** | Composed localhost validation: the kernel joins the simnet; courts pose conjectures over registered calculi; the kernel finds derivations by traversal and the books fill with DERIVED work | **DONE** — `kernel-1` is a standing node in `scripts/simnet.sh`, registered at genesis alongside solver-1/witness-1, pointed at court-a's dihedral market; `simnet.sh status` and `plumbd status` both report it correctly (derived-and-settled vs refused rounds, not misclassified as a court); live-validated on a full genesis reset of the real simnet — courts, carrier, clients, kernel, solver, witness, and gateway together on one machine, with kernel-1 crediting real axes for genuinely DERIVED work on its first round |
...
bootstrap, and signature-based admission were already real. But five
things were fixture-only and would have to be
...
- **#50 P5-A1** — the on-chain economic facts. **DONE 2026-08-29** —
  ...
  sched boundary test intact. **Boundary carried to #51:**
...
- **#53 P5-B1** — split `court_session` into handshake + settle stages;
  ...
  *(independent)*. **DONE 2026-08-28** — `court_handshake(...) ->
  ...
  HEALTHY, credits advancing, no new failure mode). *Scope note:
...
dequeue*. So the two metric terms attach at different granularities:
  ...
  **Recommendation (pending operator):** wire escrow-weighting at the
  ...
  real value now and states the gap explicitly rather than claiming full-metric
