...
**Status:** implemented — `isthmus::deed` codec, `datum::ledger`
...
This document specifies that record, so an outside implementation can
verify and append against it, not only read `IS-3`'s rendered table.
...
The digest function is chosen by the integrator, not fixed here (as in
`IS-5` §3.1): a verifier takes the function in. Fixing one would choose
a security property for every integrator.
...
- **Retraction.** There is none, by construction: a dispute is settled
  by further acts, never by removing one.
