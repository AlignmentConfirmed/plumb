...
The grain above names WHICH orbs a cell spans and nothing about how
they connect. Six orbs in a ring and six orbs fully joined are the
same grain — a hexagon crossing as a five-simplex, the recorded
defect (`defect_tag51_carries_no_shape`). `IS-1/5` closes the class:
...
`datum/tests/wire_suite.rs (mod vectors)` and asserted against the hex below. If the codec changes, that test
...
these bytes and `tests/wire_suite.rs (mod vectors)` asserts them, so the two cannot
